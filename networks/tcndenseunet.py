...
# from networks.get_layer_from_string import get_layer
...
class Conv2DActNorm(torch.nn.Module):
    ...
    def __init__(
        ...
        act = torch.nn.ELU()  # get_layer(activation)()
...
class TCNResBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.layer = torch.nn.Sequential(
            ...
            torch.nn.ELU(),  # get_layer(activation)(),
            dconv,
            point_conv,
        )
...
class TCNDenseUNet(torch.nn.Module):
    ...
    def forward(self, tf_rep):
        ...
        tf_rep = tf_rep.permute(0, 1, 3, 2)
        bsz, mics, _, frames = tf_rep.shape # B, C, F, T
        #print(bsz, mics, _, frames)
        #print(mics)
        #print(self.mic_channels)
...
def test():
    ...
    x = torch.randn((8, 1, 257, 257), dtype=torch.cfloat)
    x = x.to(device)
    model = TCNDenseUNet().to(device)
    # preds = model(x)
    # print(f'OUT: {preds.shape} {preds.dtype}')

    import torchinfo
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    torchinfo.summary(model, (1, 1, 257, 257))
