"""
Train script for VCTK dataset only
"""
import os, sys
import yaml
...
# other modules
from utils.test_val_utils import FW_inpainter_VCTK
from utils.utils import save_checkpoint, save_gen_specs, vctk_parse, count_parameters
from speech_dataset import VCTK_dataset_raw
...
def train_fn(disc, gen, loader, opt_disc, opt_gen, loss, lr_schedulers):
    ...
    for idx, (x, y) in enumerate(loop): # x = lossy, y = clean
        ...
        D_loss = 0.1*(D_real_loss + D_fake_loss) / 2
        ...
        # perceptual loss
        PMSQE_loss = torch.mean(loss[2](torch.abs(y_fake)[:,0,:,:], torch.abs(y)[:,0,:,:]))
...
def val_metrics_fn(gen, filelist, inpainter):
    ...
    gen.eval()
    for idx in tqdm(range(1,100,2)): # select a subset of validation files without shuffle
        fname = filelist[idx]
...
def main():
    ...
    # parse dataset
    train_list, val_list, test_list = vctk_parse(par["VCTK_DIR"], par["VCTK_META"])

    # define dataset and dataloader
    train_dataset = VCTK_dataset_raw(train_list, par, 'train')
    val_dataset = VCTK_dataset_raw(val_list, par, 'val')
    
    val_I = FW_inpainter_VCTK(par, test_plr = 0.4)
    ...
    visual_proof_sample = val_dataset[10] 
    ...
    for epoch in range(par["NUM_EPOCHS"]):
        ...
        writer.add_scalar('PESQ_VAL',       (val_metrics_result[0]), epoch) # validation PESQ
        writer.add_scalar('STOI_VAL',       (val_metrics_result[1]), epoch) # validation STOI
        writer.add_scalar('PLCMOS_VAL',     (val_metrics_result[2]), epoch) # validation PLCMOS
        writer.flush()
        ...
        if current_P > best_P:
            ...
            print(f'> best PESQ checkpoint saved ({current_P:.3f})')
        ...
        if current_S > best_S:
            ...
            print(f'> best STOI checkpoint saved ({current_S:.3f})')
        ...
        if current_PLCMOS > best_PLCMOS:
            ...
            print(f'> best PLCMOS checkpoint saved ({current_PLCMOS:.3f})')
